		...
		methods: {
			getData() {
				...
				uni.getStorage({
					key: 'seal_params',
					success(e) {
						helper.request('api/user/get_my_address', {
							id: that.id
							...
							if (res.data.code == 1000) {
								...
								that.selectList = [{txt:data.location_p},{txt:data.location_c},{txt:data.location_a}];
								that.name = data.name;
								that.phone = data.phone;
								that.address = data.address;
								that.location_p = data.location_p;
								that.location_c = data.location_c;
								that.location_a = data.location_a;
							} else {
								uni.showToast({
									title: res.data.msg,
									icon: 'none'
						...
						success(e) {
							helper.request('api/user/save_address', {
								...
								if (res.data.code == 1000) {
									if(that.sort == 1){
										uni.redirectTo({
											url:'/pages/address/address?sort=1' + '&id=' + that.pid + '&formtype=' + that.formtype + '&num=' + that.num
										})
									}else if(that.sort == 2){
										console.log(222)
										uni.redirectTo({
